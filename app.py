...
fp_num = sp.lambdify(x, fprime_expr, modules=["numpy"])

if fdoubleprime is not None:
    fpp_num = sp.lambdify(x, fdoubleprime, modules=["numpy"])
...
if fdoubleprime is not None:
    fpp_vals = fpp_num(x_vals)
    if np.isscalar(fpp_vals):
        fpp_vals = np.full_like(x_vals, fpp_vals, dtype=float)
    axes[2].plot(x_vals, fpp_vals)
